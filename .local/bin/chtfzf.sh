...
function getPath {
  echo "$(dirs -p | tail -n +2 | sed 's/.$//' | sed '1!G;h;$!d' | tr '\n' '/')"
}

function openSheet {
  if [[ "$1" == "" ]]; then exit; fi # Exit if empty string
  case "$openMode" in
  tmux) tmux popup -E -w 80% -h 80% bash -c "curl -sg "cht.sh/$*" | BAT_PAGER='less -R' bat" ;;
  bash) curl -sg "cht.sh/$*" | BAT_PAGER="less" bat ;;
